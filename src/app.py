...
def _normalize_lab_value(metric: str, value: float, detected_unit: str) -> tuple[float, str, str]:
    ...
    def auto_decimal_if_needed(raw_value: float, lower_bound: float, upper_bound: float) -> tuple[float, bool]:
        if lower_bound <= raw_value <= upper_bound:
            return raw_value, False
        if raw_value > upper_bound and (raw_value / 10.0) >= lower_bound and (raw_value / 10.0) <= upper_bound:
    ...
    if metric in {"Hemoglobin", "MCHC"}:
        ...
        if metric == "Hemoglobin":
            corrected, changed = auto_decimal_if_needed(value, 5.0, 25.0)
            if changed:
                return corrected, "g/dL", "auto decimal correction"
        else:
            corrected, changed = auto_decimal_if_needed(value, 20.0, 45.0)
            if changed:
                return corrected, "g/dL", "auto decimal correction"
    ...
    if metric == "MCH":
        ...
        corrected, changed = auto_decimal_if_needed(value, 15.0, 45.0)
        if changed:
            return corrected, "pg", "auto decimal correction"

        return value, "pg", "standardized"

    if metric == "MCV":
        corrected, changed = auto_decimal_if_needed(value, 50.0, 130.0)
        if changed:
            return corrected, "fL", "auto decimal correction"
